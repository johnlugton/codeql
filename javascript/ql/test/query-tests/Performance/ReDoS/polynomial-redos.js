...
app.use(function(req, res) {
	...
	tainted.match(/^(\+|-)?(\d+|(\d*\.\d*))?(E|e)?([-+])?(\d+)?$/); // NOT OK6
	...
	tainted.match(/B?(YH|K)(YH|J)*X/); // NOT OK - but not detected

	tainted.match(/a*b/); // NOT OK - the initial repetition can start matching anywhere.
	tainted.match(/cc*D/); // NOT OK - the repetition overlaps with the previous (and the previous can start anywhere)
